"""Google Sheet 旧版 Jinja 页面路由（整文件停用）。

单 Token 子服务模式（2026-09 起）:

- 静态模板前端已整体下线：页面统一由 Vue 前端提供
  （C3/C4/C5/C7/C31 页面对应 ``frontend/src/views/task/*``），
  且浏览器导航无法携带 ``Token`` 请求头，旧页面在新鉴权下本就不可达。
- 任务业务数据接口在 ``app/routes/google_sheet_api.py``（/api 前缀），
  与本文件无关。恢复旧模板页面时取消下方注释即可。
"""
