...
from app.utils.auth import is_auth_enabled, login_required
...
def export_permission_required(permission_code: str):
    """导出接口统一权限校验；认证关闭时遵循开发环境放行约定。"""
    def decorator(view):
        @wraps(view)
        def wrapped(*args, **kwargs):
            if is_auth_enabled():
                user = getattr(g, "current_user", None)
                permissions = set(user.get_permissions() if user else ())
                if permission_code not in permissions:
                    return jsonify({
                        "code": 403,
                        "data": {"required_permissions": [permission_code]},
...
def _permission_error(task: Task, task_id: str):
    decision = authorize_task_type_action(getattr(g, "current_user", None), "view", task.task_type)
    if decision["allowed"]:
        return None
    missing = "、".join(decision.get("missing_permissions") or []) or "未知"
    return jsonify({
        "status": "error",
        "message": f"权限不足，无法导出任务；当前缺少: {missing}",
        "task_id": task_id,
        "task_type": decision.get("task_type") or task.task_type,
...
def _load_task(task_id: str):
    ...
    denied = _permission_error(task, task_id)
    return (None, denied) if denied else (task, None)
...
@login_required
@export_permission_required("task:view")
...
def export_task_results_batch():
    try:
        ...
        for task in tasks:
            decision = authorize_task_type_action(current_user, "view", task.task_type)
            if not decision["allowed"]:
                return _permission_error(task, task.id)
...
@login_required
@export_permission_required("backtest:view")
...
def export_global_preview_batch():
    try:
        ...
        for task in tasks:
            denied = _permission_error(task, task.id)
            if denied:
                return denied
