...
数据库直连迁移状态（单 Token 子服务模式，2026-09 起）:

- 本服务不再维护本地导航菜单与本地 RBAC：菜单仅由 ``/api/meta/nav``
  拉取远程 ``GetUserRoleList`` 提供（见 ``app/routes/meta_api.py``）。
- 以下默认菜单常量、权限同步与菜单树组装逻辑已整体注释停用，
  原 ``Permission`` ORM 同步依赖的本地 RBAC 模型一并注释
  （见 ``app/models.py``）。恢复本地导航管理时取消注释即可。
"""
