"""RBAC 认证态与用户/角色/权限服务（数据层：auth_repository + task_repository）。
...
from app.repositories import navigation_repository, auth_repository, task_repository
...
def _can_alert_oncall(role_ids=None, user=None):
    if role_ids is not None:
        ...
        return any(
            ...
            for role in auth_repository.list_roles_by_ids(role_ids)
        )
    user_data = user
...
def login_user(username: str, password: str) -> dict:
    ...
    credentials = auth_repository.get_user_credentials(username)
    ...
    auth_repository.update_last_login(credentials["id"], datetime.now())

    user = auth_repository.get_user(credentials["id"], include_permissions=True)
...
def refresh_tokens(token: str) -> dict:
    ...
    user = auth_repository.get_user(payload['user_id'], include_permissions=True)
    if not user or not user["is_active"]:
        raise UnauthorizedError("用户不存在或已禁用")
    state = auth_repository.get_user_state(payload['user_id'])
...
def change_password(user, old_pwd: str, new_pwd: str) -> None:
    ...
    auth_repository.update_user(user.id, {
...
def update_user(user_id: int, data: dict) -> dict:
    ...
    if 'password' in data and data['password']:
        ...
        state = auth_repository.get_user_state(user_id)
    ...
    if 'is_alert_oncall' in data or 'role_ids' in data:
        refreshed = auth_repository.get_user(user_id)
        alert_flag = bool(
            data.get('is_alert_oncall', refreshed["is_alert_oncall"])
        ) and _can_alert_oncall(user=refreshed)
        updated = auth_repository.update_user(user_id, {"is_alert_oncall": alert_flag})
...
def delete_user(user_id: int) -> None:
    ...
    if not auth_repository.get_user_state(user_id):
        raise NotFoundError("用户不存在")
    with auth_repository.transaction():
        task_repository.clear_created_by(user_id, commit=False)
        auth_repository.delete_user(user_id, commit=False)
...
def list_roles() -> list:
    return auth_repository.list_roles(include_permissions=True)
...
def create_role(*, name: str, code: str, permission_ids, description) -> dict:
    ...
    if auth_repository.role_code_exists(code):
        raise ValidationError("角色编码已存在")

    created = auth_repository.create_role(
        code,
        name,
...
def update_role(role_id: int, data: dict) -> dict:
    role = auth_repository.get_role(role_id)
    if not role:
...
def list_permissions_grouped() -> dict:
    ...
    grouped = {}
    for perm in auth_repository.list_permissions():
