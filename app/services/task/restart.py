...
from app.models import Task, TaskLog, TaskResult, TaskResultReturn, TaskResultSummaryIndex, XplAnalysisJob
...
def _delete_return_series_archive(task_id: str) -> None:
    try:
        deleted = ReturnSeriesService().delete_task_archive(task_id)
        if deleted:
            logger.info("已删除任务收益序列归档文件: %s", task_id)
    except Exception as exc:
        logger.warning("删除任务收益序列归档文件失败: %s, 错误: %s", task_id, exc)


def _cancel_xpl_analysis_jobs(task_id: str) -> None:
    try:
        cancelled = XplAnalysisJobService().cancel_jobs_for_task(task_id)
        if cancelled:
            logger.info("已取消任务XPL异步分析job: %s, count=%s", task_id, cancelled)
    except Exception as exc:
        logger.warning("取消任务XPL异步分析job失败: %s, 错误: %s", task_id, exc)
...
class TaskRestartMixin:
    ...
    def cancel_task(self, task_id: str) -> bool:
        ...
        _cancel_xpl_analysis_jobs(task_id)
