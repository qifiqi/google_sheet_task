"""Benchmark XPL calculation with processes using tests/test/d.py data.

Usage:
    python scripts/benchmark_xpl_process_from_d.py --jobs 12 --workers 4
    python scripts/benchmark_xpl_process_from_d.py --jobs 24 --workers 4 --skip-serial
"""
...
def parse_return_rows(data_text: str) -> ReturnRows:
    ...
    rows = XPLAnalyzer()._parse_input_data(data_text)
    if not rows:
...
def run_processes(rows: ReturnRows, jobs: int, workers: int) -> RunSummary:
    started = time.perf_counter()
    ...
    return summarize("processes", jobs, workers, time.perf_counter() - started, job_seconds)
...
def print_summaries(summaries: list[RunSummary]) -> None:
    ...
    processes = next((item for item in summaries if item.mode == "processes"), None)
    if serial and processes:
        speedup = serial.wall_seconds / processes.wall_seconds if processes.wall_seconds else 0.0
        print()
        print(f"processes speedup vs serial: {speedup:.2f}x")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark XPL process concurrency with tests/test/d.py data.")
    ...
    parser.add_argument("--workers", type=int, default=min(4, os.cpu_count() or 1), help="Process workers.")
    parser.add_argument("--skip-serial", action="store_true", help="Only run the process benchmark.")
...
def main() -> None:
    ...
    summaries.append(run_processes(rows, args.jobs, args.workers))
