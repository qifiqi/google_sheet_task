...
def _kline_rows(start_date, end_date):
    """按工作日粒度生成 K 线，贴近真实行情密度（避免触发按 limit 截取最新行）。"""
...
def _search_result(market, code="600000"):
    """构造符合 StockSearchService._normalize_result 契约的东方财富搜索结果。"""
    return [{
        "status": 10,
        "code": code,
        "market": market,
        "securityTypeName": "A股" if market in ("0", "1") else "美股",
        "shortName": "测试股票",
    }]
...
def test_c3_allows_historical_end_date_before_latest_kline(app_factory):
    with app_factory.app_context():
        service = C3GoogleSheetService({}, "task-id")
        service.google_sheet = _FakeSheet()
        service.dfcf_api.get_search_list_by_stock_code = lambda *_args, **_kwargs: _search_result("1")
        service.dfcf_api.get_stock_kline_data = (
            lambda *_args, **_kwargs: _kline_rows("2023-04-24", "2024-05-10")
        )
...
def test_c3_vwap_uses_dfcf_for_en_market_and_writes_vwap(app_factory):
    with app_factory.app_context():
        service = C3GoogleSheetService({}, "task-id")
        service.google_sheet = _FakeSheet()
        ...
        service.dfcf_api.get_search_list_by_stock_code = lambda *_args, **_kwargs: _search_result("105")
        service.dfcf_api.get_stock_kline_data = (
            lambda *_args, **_kwargs: _kline_rows("2023-04-24", "2024-05-10")
        )
...
def test_c3_rejects_end_date_after_latest_kline_without_writing_sheet(app_factory):
    with app_factory.app_context():
        service = C3GoogleSheetService({}, "task-id")
        service.google_sheet = _FakeSheet()
        service.dfcf_api.get_search_list_by_stock_code = lambda *_args, **_kwargs: _search_result("1")
...
def test_c3_empty_kline_does_not_write_sheet(app_factory):
    with app_factory.app_context():
        service = C3GoogleSheetService({}, "task-id")
        service.google_sheet = _FakeSheet()
        service.dfcf_api.get_search_list_by_stock_code = lambda *_args, **_kwargs: _search_result("1")
...
def test_c5_recent_mode_skips_empty_ranges_and_raises_when_none_left(app_factory):
    with app_factory.app_context():
