...
function inject() {
  try {
    // ZCode adaptation: ZCode has no SubagentStart event; this hook is wired
    // under PreToolUse (matcher Agent|Task, mirroring the upstream Qoder
    // adapter). The JSON envelope must name the event it actually runs under
    // or ZCode's strict output validator drops the context.
    writeHookOutput('PreToolUse', mode, getPonytailInstructions(mode));
  } catch (e) {
