// Google Sheet 3.1 批量创建页（templates/google_sheet_c31/create.html 的页面逻辑）。
// 自 google_sheet_c31/create.html 内联脚本原样抽离；接口调用经 common/api.js。
    const TASK_TYPE = 'google_sheet_C31';
    const STORAGE_KEY = 'google_sheet_c31_form_data';
    const PAGE_LABEL = '3.1';
    ...
    async function loadStockMarkets(selectedValue = 'cn') {
        ...
        select.value = normalizeMarketTypeValue(selectedValue);
    }
    const GOOGLE_SHEET_TABLE_TYPE = 'c3';
    ...
    function getParameterValues() {
        return getParameterIds().map((paramId, index) => {
            const field = document.getElementById(paramId);
            const rawValue = field ? field.value.trim() : '';
            if (!rawValue) {
                return { id: paramId, index: index + 1, value: [], empty: true };
            }
    ...
    function validateC31Parameters(parameterValues, showToast = true) {
        for (const param of parameterValues) {
            if (param.value === null) {
                if (showToast) {
                    showNotification(`参数${param.index} 不是合法的 JSON 数组`, 'error');
                }
                return false;
            }
            if (!Array.isArray(param.value)) {
                if (showToast) {
                    showNotification(`参数${param.index} 必须是数组`, 'error');
                }
                return false;
            }
        }
    ...
    function getActiveParameters(parameterValues) {
        return parameterValues
            .filter(param => Array.isArray(param.value) && param.value.length > 0)
            .map(param => param.value);
    }

    function getC31ParameterCandidateGroups(parameterValues) {
        const activeParameters = getActiveParameters(parameterValues);
        // 与后端 _normalize_c31_parameter_groups 保持一致：
        // 二维数组的每一行都是一个候选项；一维数组整体作为单个候选项。
    ...
    function getParamCandidateCount(paramArray) {
        if (!Array.isArray(paramArray) || paramArray.length === 0) return 0;
        if (Array.isArray(paramArray[0])) {
            return paramArray[0].length;
        }
        return paramArray.length;
    }

    function getC31ParameterCombinationCount(parameterValues) {
        const activeParameters = getActiveParameters(parameterValues);
        if (!activeParameters.length) {
            return 0;
        }
        return activeParameters.reduce((total, group) => total * getParamCandidateCount(group), 1);
    }

    function isCountCompatible(leftCount, rightCount) {
        if (leftCount <= 0 || rightCount <= 0) {
            return false;
        }
        return leftCount === rightCount || leftCount % rightCount === 0 || rightCount % leftCount === 0;
    }

    function getSelectedSheetCount() {
        ...
        let count = 0;
        sheetConfigList.querySelectorAll('.sheet-config-item').forEach(item => {
            const { spreadsheetSelect } = getSheetConfigElements(item);
            const spreadsheetId = spreadsheetSelect ? extractSpreadsheetId(spreadsheetSelect.value || '') : '';
            if (spreadsheetId) {
                count += 1;
            }
        });
    ...
    function isValidC31SheetTitle(title) {
        return /.+-\d+y-\d+\]$/.test(String(title || '').trim());
    }

    function validateSelectedSheetTitles(showToast = true) {
        const sheetConfigList = document.getElementById('sheet-config-list');
        if (!sheetConfigList) {
            return true;
        }
        ...
        for (let index = 0; index < items.length; index += 1) {
            ...
            const { spreadsheetSelect, titleInput } = getSheetConfigElements(item);
            const spreadsheetId = spreadsheetSelect ? extractSpreadsheetId(spreadsheetSelect.value || '') : '';
            if (!spreadsheetId) {
                continue;
            }
            ...
            if (!isValidC31SheetTitle(title)) {
                if (showToast) {
                    showNotification(`第${index + 1}个Sheet的表标题格式无效，必须以“任意前缀-数字y-数字]”结尾，例如：策略A-1y-3]`, 'error');
                }
                if (titleInput) {
                    titleInput.focus();
                }
    ...
    function getSelectedRecentYears() {
        return Array.from(document.querySelectorAll('.recent-year-checkbox:checked'))
            .map(item => Number(item.value))
            .filter(value => Number.isFinite(value));
    }

    function getRecentYearsPayload() {
        ...
        if (countMode !== 'n_plus_1' || !recentEl || !recentEl.checked) {
            return null;
        }

        const selected = getSelectedRecentYears();
        return selected.length > 0 ? selected : 'all';
    }

    function applyRecentYears(raw) {
        document.querySelectorAll('.recent-year-checkbox').forEach(item => {
            item.checked = false;
        });

        if (raw === 'all' || raw === null || raw === undefined) {
            return;
        }

        const values = Array.isArray(raw) ? raw.map(Number) : [Number(raw)];
        document.querySelectorAll('.recent-year-checkbox').forEach(item => {
            item.checked = values.includes(Number(item.value));
        });
    }

    function updateRecentYearsVisibility() {
        const container = document.getElementById('recent-years-container');
        const countMode = document.querySelector('input[name="count_mode"]:checked')?.value || 'total';
        const recentEl = document.getElementById('date_range_recent');
        if (!container || !recentEl) {
            return;
        }

        const visible = countMode === 'n_plus_1' && recentEl.checked;
        container.style.display = visible ? 'block' : 'none';
    }

    function refreshDateRangeModeAvailability() {
        ...
        updateRecentYearsVisibility();
    }

    function getStockCode() {
        const input = document.getElementById('stock_code_input');
    ...
    function loadFromTemplate(template) {
        ...
        try {
            ...
            if (template.config) {
                ...
                // “更多配置”已移除，暂不回填这些字段；保留结构供后续重新接后端时恢复。
                ...
                // 更新页面标题和提示
                // （原模板此处有一个按 template_id 条件渲染的标题更新块；服务端从未向模板
                //   注入 template_id 上下文变量，该分支在任何 URL 下都未渲染，属死代码，
                //   按原服务端渲染结果原样移除——标题更新逻辑仅 C3 页存在且为运行时判断。）
            }
        } catch (error) {
    ...
    document.addEventListener('DOMContentLoaded', async function() {
        ...
        bindEvents();
        loadGoogleSheets();
        // “更多配置”已移除，暂不初始化 Token 列表；保留函数供后续恢复。
        // loadGoogleSheetTokens();
        ...
        normalizeBatchBasicFieldsMarkup();
    ...
    function normalizeBatchBasicFieldsMarkup() {
        const endDateInput = document.getElementById('end_date');
        const currentEndDate = endDateInput ? endDateInput.value : '';
        if (endDateInput) {
            const endDateCol = endDateInput.closest('.col-md-3');
            if (endDateCol) {
                endDateCol.innerHTML = `
                    <label for="end_date" class="form-label">结束时间</label>
                    <input type="date" class="form-control" id="end_date">
                    <div class="form-text">默认使用最近工作日</div>
                `;
                const rebuiltEndDateInput = endDateCol.querySelector('#end_date');
                if (rebuiltEndDateInput) {
                    rebuiltEndDateInput.value = currentEndDate;
                }
            }
        }

        const currentPriceMode = document.getElementById('price_mode')?.value || 'vwap_price';
        const currentAdjustment = document.getElementById('kline_adjustment')?.value || 'forward';
        if (!document.getElementById('price_mode')) {
            const marketTypeCol = document.getElementById('market_type')?.closest('.col-md-2');
            const priceModeCol = document.createElement('div');
            priceModeCol.className = 'col-md-3 mb-3';
            priceModeCol.innerHTML = `
                <label class="form-label" for="price_mode">价格类型</label>
                <select class="form-select" id="price_mode">
                    <option value="vwap_price">加权平均价</option>
                    <option value="kp_price">开盘价</option>
                    <option value="sp_price">收盘价</option>
                </select>
                <div class="form-text">写入 K 线价格列的数据口径</div>
            `;
            if (marketTypeCol && marketTypeCol.parentElement) {
                marketTypeCol.parentElement.insertBefore(priceModeCol, marketTypeCol.nextSibling);
            }
            const priceModeSelect = priceModeCol.querySelector('#price_mode');
            if (priceModeSelect) priceModeSelect.value = currentPriceMode;
        }
        if (!document.getElementById('kline_adjustment')) {
            const marketTypeCol = document.getElementById('market_type')?.closest('.col-md-2');
            const adjustmentCol = document.createElement('div');
            adjustmentCol.className = 'col-md-2 mb-3';
            adjustmentCol.innerHTML = `
                <label class="form-label" for="kline_adjustment">K线复权</label>
                <select class="form-select" id="kline_adjustment">
            ...
            `;
            if (marketTypeCol && marketTypeCol.parentElement) {
                marketTypeCol.parentElement.insertBefore(adjustmentCol, marketTypeCol.nextSibling);
            }
            const adjustmentSelect = adjustmentCol.querySelector('#kline_adjustment');
            if (adjustmentSelect) adjustmentSelect.value = currentAdjustment;
        }
        if (!document.getElementById('kline_data_source')) {
            const marketTypeCol = document.getElementById('market_type')?.closest('.col-md-2');
            if (marketTypeCol) {
                const sourceCol = document.createElement('div');
                sourceCol.className = 'col-md-2 mb-3';
                sourceCol.innerHTML = `
                ...
                const adjustmentCol = document.getElementById('kline_adjustment')?.closest('.col-md-2, .col-md-3');
                marketTypeCol.parentElement.insertBefore(sourceCol, adjustmentCol?.nextSibling || marketTypeCol.nextSibling);
            }
        }
    }
    ...
    function loadTemplates() {
        Api.endpoints.template.list(`task_type=${TASK_TYPE}`)
            .then(data => {
    ...
    function fillFormWithTemplate(templateId) {
        ...
        Api.endpoints.template.detail(templateId)
            .then(template => {
                let config = template.config;
                console.log('[C31 fillFormWithTemplate] config =', config);
                ...
                if (template && template.description && !config.task_description) {
                    config.task_description = template.description;
                }
    ...
    function applyConfigToForm(rawConfig, options = {}) {
        ...
        if (normalized.base_task_name) {
            document.getElementById('task_name').value = normalized.base_task_name;
        }
        ...
        // }
        // refreshDateRangeModeAvailability();
        // applyRecentYears(normalized.recent_years);
        // updateRecentYearsVisibility();
        if (normalized.market_type) document.getElementById('market_type').value = normalizeMarketTypeValue(normalized.market_type);
        ...
        const endDateInput = document.getElementById('end_date');
        if (endDateInput) {
            endDateInput.value = normalized.end_date || normalized.start_date || '';
        }
        ...
        if (Array.isArray(normalized.stock_codes) && normalized.stock_codes.length > 0) {
            const input = document.getElementById('stock_code_input');
            if (input) {
                input.value = String(normalized.stock_codes[0]).trim();
            }
        }

        // mode-specific title/header
        if (mode === 'restart') {
            document.title = '重启任务 - Google Sheet 3.1 批量启动';
    ...
    function initDefaultDatesIfEmpty() {
        ...
        endInput.value = TradingDate.formatDate(TradingDate.previousWeekday());
    }
    ...
    function bindEvents() {
        ...
        countModeRadios.forEach(r => {
            r.addEventListener('change', refreshDateRangeModeAvailability);
        });
        [drmFull, drmRecent].forEach(item => {
            if (item) {
        ...
        document.querySelectorAll('.recent-year-checkbox').forEach(item => {
            item.addEventListener('change', debouncedSaveFormData);
        });
        refreshDateRangeModeAvailability();
        ...
        // “更多配置”已移除，先注释旧事件绑定逻辑，后续若恢复后端传参可直接启用。
        // const tokenTypeEl = document.getElementById('token_type');
        // if (tokenTypeEl) {
        //     tokenTypeEl.addEventListener('change', function() {
        ...
        if (importTokenBtn) {
            importTokenBtn.addEventListener('click', importGoogleSheetToken);
        }
        ...
        if (stockCodeInput) {
            stockCodeInput.addEventListener('input', debouncedSaveFormData);
        }

        // 为所有输入字段添加自动保存
        const inputFields = [
            'task_name', 'task_description', 'stock_code_input', 'end_date', 'price_mode', 'kline_adjustment'
        ];
    ...
    // 计算参数组合数量（Google Sheet 数量 * 参数数组长度乘积）
    function calculateCombinations() {
        const parameterValues = getParameterValues();
        const sheetCount = getSelectedSheetCount();
        const stockCodeValue = getStockCode();
        let count = 0;
        let parameterCombinationCount = 0;

        if (sheetCount > 0 && stockCodeValue && validateC31Parameters(parameterValues, false)) {
            parameterCombinationCount = getC31ParameterCombinationCount(parameterValues);
            if (parameterCombinationCount > 0) {
                count = parameterCombinationCount * sheetCount;
            }
        }

        const infoDiv = document.getElementById('combination-info');
        const countSpan = document.getElementById('combination-count');
        const singleCountSpan = document.getElementById('single-combination-count');
        const sheetCountSpan = document.getElementById('sheet-count-display');
        const stockCountSpan = document.getElementById('stock-count-display');

        if (count > 0) {
            infoDiv.style.display = 'block';
            countSpan.textContent = count;
            singleCountSpan.textContent = parameterCombinationCount;
            sheetCountSpan.textContent = sheetCount;
            stockCountSpan.textContent = 1;
        } else {
    ...
    function showCombinationPreview() {
        const parameterValues = getParameterValues();
        if (!validateC31Parameters(parameterValues)) {
            return;
        }
        if (!validateSelectedSheetTitles(true)) {
            return;
        }

        const parameterGroups = getC31ParameterCandidateGroups(parameterValues).filter(item => Array.isArray(item) && item.length > 0);
        const parameterCombinationCount = getC31ParameterCombinationCount(parameterValues);
        const sheetCount = getSelectedSheetCount();
        if (!isCountCompatible(parameterCombinationCount, sheetCount)) {
            showNotification(`参数组合数(${parameterCombinationCount})与Sheet数(${sheetCount})必须相等，或其中一方是另一方的整数倍`, 'error');
            return;
        }

        const totalCount = calculateCombinations();
        const combinations = generateCombinations(parameterGroups, 20);

        const previewContainer = document.getElementById('combination-preview');
        previewContainer.innerHTML = '';

        const stockCodeBlock = document.createElement('div');
        stockCodeBlock.className = 'mb-3 p-2 border rounded bg-light';
        stockCodeBlock.innerHTML = `
            <strong>股票代码:</strong>
            <div class="small text-muted"><code>${getStockCode()}</code></div>
        `;
        previewContainer.appendChild(stockCodeBlock);

        combinations.forEach((combination, index) => {
            const div = document.createElement('div');
            div.className = 'mb-2 p-2 border rounded';
            div.innerHTML = `
                <strong>参数组合 ${index + 1}:</strong>
                <div class="small text-muted"><code>${JSON.stringify(combination)}</code></div>
            `;
            previewContainer.appendChild(div);
        });

        if (totalCount > combinations.length) {
            const moreDiv = document.createElement('div');
            moreDiv.className = 'text-center text-muted';
            moreDiv.textContent = `... 还有 ${totalCount - combinations.length} 个组合`;
    ...
    // 生成参数组合（笛卡尔积）
    function generateCombinations(parameters, limit = Number.POSITIVE_INFINITY) {
        if (!Array.isArray(parameters) || parameters.length === 0) {
            return [];
        }

        return parameters.reduce((accumulator, currentGroup) => {
    ...
    function clearAllParameters() {
        if (confirm('确定要清空所有配置吗？')) {
            getParameterIds().forEach(paramId => {
                const field = document.getElementById(paramId);
                if (field) {
                    field.value = '';
                }
    ...
    function submitTask() {
        ...
        // const recentYears = getRecentYearsPayload();
        ...
        if (!taskName) {
            showNotification('请输入任务 Base Name', 'error');
            return;
        }
        const stockCodeValue = getStockCode();
        if (!stockCodeValue) {
        ...
        const marketType = normalizeMarketTypeValue(document.getElementById('market_type')?.value);
        const priceMode = document.getElementById('price_mode')?.value || 'vwap_price';
        const klineAdjustment = document.getElementById('kline_adjustment')?.value || 'forward';
        ...
        const parameters = getActiveParameters(parameterValues);

        const sheets = collectSheetConfigs();

        // 构造任务配置（C5 任务仅使用 sheets 数组，不再重复顶层 spreadsheet_id/sheet_name）
        const taskConfig = {
            base_task_name: taskName,
            task_description: taskDescription,
            stock_codes: [stockCodeValue],
        ...
        Api.endpoints.task.batchCreate(taskData).then(function(data) {
            ...
            // 延迟1秒后跳转到 C3 任务列表页
            setTimeout(function() {
                window.location.href = `/google-sheet/?version=c3`;
            }, 1000);
        }).catch(function(err) {
            executeBtn.disabled = false;
            executeBtn.innerHTML = '<i class="bi bi-play-circle"></i> 创建任务并执行';
    ...
    function loadSavedFormData() {
        try {
            ...
            if (savedData) {
                ...
                if (formData.stock_code) document.getElementById('stock_code_input').value = formData.stock_code;
                if (typeof formData.end_date === 'string') {
                    document.getElementById('end_date').value = formData.end_date;
                } else if (typeof formData.start_date === 'string') {
                    document.getElementById('end_date').value = formData.start_date;
                }
                if (Array.isArray(formData.sheets) && formData.sheets.length > 0) {
                    applySheetsToForm(formData.sheets);
                } else if (formData.spreadsheet) {
                ...
                if (formData.param4 && param4El) param4El.value = formData.param4;
                const param5El = document.getElementById('param5');
                if (formData.param5 && param5El) param5El.value = formData.param5;
                const param6El = document.getElementById('param6');
                if (formData.param6 && param6El) param6El.value = formData.param6;

                if (formData.market_type) document.getElementById('market_type').value = normalizeMarketTypeValue(formData.market_type);
                ...
                // }
                // refreshDateRangeModeAvailability();
                // applyRecentYears(formData.recent_years);
                // document.getElementById('token_type').dispatchEvent(new Event('change'));
                calculateCombinations();
    ...
    function clearSavedFormData() {
        try {
            ...
            document.getElementById('combination-info').style.display = 'none';
            calculateCombinations();
            showNotification('已清除 3.1 表单缓存', 'success');
        } catch (e) {
    ...
    function getCurrentConfig() {
        try {
            const baseTaskName = document.getElementById('task_name').value.trim();
            const taskDescription = document.getElementById('task_description').value.trim();
            const stockCodeValue = getStockCode();
            const marketType = normalizeMarketTypeValue(document.getElementById('market_type')?.value);
            const priceMode = document.getElementById('price_mode')?.value || 'vwap_price';
            ...
            if (!baseTaskName) {
                showError('请输入任务 Base Name');
                return null;
            }
            if (!stockCodeValue) {
                showError('请至少添加一个股票代码');
                return null;
            }

            // 验证必要字段
            if (getSelectedSheetCount() === 0) {
                showError('请选择 Google Sheet');
                return null;
            }
            ...
            if (endDate) {
                currentConfig.end_date = endDate;
            }
    ...
    function submitTemplate() {
        ...
        Api.endpoints.template.create(templateData)
        .then(data => {
    ...
    function normalizeC4Config(raw) {
        ...
        return {
            base_task_name: raw.base_task_name || raw.task_name || '',
            ...
            stock_codes: Array.isArray(raw.stock_codes) ? raw.stock_codes : [],
            recent_years: raw.recent_years ?? null,
