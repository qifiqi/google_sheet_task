// Google Sheet 创建任务页（C4）（templates/google_sheet_c4/create.html 的页面逻辑）。
// 批内收敛：同名函数已提升至 static/js/common/business/（Biz.*），见 F3 收敛 pass。
// 自 templates/google_sheet_c4/create.html 内联脚本原样抽离；接口调用经 common/api.js。
...
if (versionParam && versionParam !== 'c4') {
    ...
    function loadTemplates() {
        // 只加载 C4 类型的模板（后端按 config.task_type 精确匹配，必须传 task_type= 查询串）
        Api.endpoints.template.list('task_type=google_sheet_C4')
            .then(data => {
    ...
    function submitTask() {
        ...
        const marketType = document.getElementById('market_type')?.value || 'cn';
    ...
    function getCurrentConfig() {
        try {
            ...
            // 验证必要字段
            if (!spreadsheetId) {
                Biz.formState.showError('请输入电子表格ID或URL');
                return null;
            }

            if (sheetNameSelect === '自定义' && !customSheetName) {
                Biz.formState.showError('请输入自定义工作表名称');
                return null;
            }
            ...
            // 检查是否有解析错误
            if (param1 === null) {
                Biz.formState.showError('参数格式错误，请检查JSON格式');
                return null;
            }
