...
// 接口调用经 common/api.js。
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('btn-copy-daily-returns')?.addEventListener('click', copyDailyReturns);
    ...
    function buildDetailHref() {
        ...
        return `/backtest-multi-product/detail/${encodeURIComponent(state.taskId)}${query ? `?${query}` : ''}`;
    }
    ...
    async function copyDailyReturns() {
        const text = state.dailyReturnsJsonText || '';
        if (!text) {
            Biz.showAlert('暂无可复制内容', 'warning');
            return;
        }
    ...
    async function loadBacktestResult() {
        ...
        try {
            const data = await Api.endpoints.backtestMulti.taskResult(encodeURIComponent(state.resultId), {
            ...
            state.exportBaseName = `backtest_result_${state.resultId}`;
    ...
    async function exportWordReport() {
        if (!state.wordReportPayload) {
            Biz.showAlert('当前参数方案缺少完整的多品收益序列', 'warning');
            return;
        }
    ...
    function renderTabDailyReturns(r) {
        const tbody = document.getElementById('tbl-daily-returns');
        if (!tbody) return;
        tbody.innerHTML = '';

        const raw = r.daily_returns;
        let rows = [];
        let jsonText = '';
        ...
        state.dailyReturnsJsonText = jsonText;

        if (!rows.length) {
            if (typeof raw === 'string' && raw.trim()) {
                tbody.innerHTML = `<tr><td colspan="3"><pre class="mb-0 text-wrap" style="white-space: pre-wrap; word-break: break-word;">${Biz.escapeHtml(raw)}</pre></td></tr>`;
            } else {
                tbody.innerHTML = '<tr><td colspan="3" class="text-center text-body-secondary">暂无数据</td></tr>';
            }
            return;
        }
        ...
        rows.forEach(item => {
            ...
            tr.innerHTML = `
                <td>${Biz.escapeHtml(String(item.date ?? '-'))}</td>
                <td class="${(Number(item.index_return) || 0) >= 0 ? 'text-success' : 'text-danger'}">${Biz.fmtPct(item.index_return, 2)}</td>
                <td class="${(Number(item.start_return) || 0) >= 0 ? 'text-success' : 'text-danger'}">${Biz.fmtPct(item.start_return, 2)}</td>
            `;
