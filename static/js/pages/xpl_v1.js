// 页面脚本（templates/xpl/v1.html 内联脚本原样抽离，F5 de-jinja）。
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('btn-analyze-v1').addEventListener('click', runAnalyzeV1);
    ...
    function showLoading() {
        ...
        const btnAnalyze = document.getElementById('btn-analyze-v1');
    ...
    function hideLoading() {
        ...
        if (btnAnalyze) btnAnalyze.disabled = !hasSheets;
    ...
    function resetWorksheetSelect() {
        ...
        document.getElementById('btn-analyze-v1').disabled = true;
    }
    ...
    async function fetchWorksheets(silent = false) {
        ...
        try {
            ...
            document.getElementById('btn-analyze-v1').disabled = false;
    ...
    async function exportV1Details() {
        ...
        try {
            ...
            const defaultFilename = `${filenameSafeTitle}_${filenameSafeSheet}_details.csv`;
            ...
            if (window.showSaveFilePicker && location.protocol === 'https:' && navigator.userAgent.includes('Chrome')) {
                try {
                    ...
                    showAlert('文件已保存', 'success');
                    return;
                } catch (err) {
                    if (err.name !== 'AbortError') {
                        console.warn('File System Access API failed, falling back to download:', err);
                    } else {
                        showAlert('用户取消了保存操作', 'info');
                        return;
                    }
                }
            }
            ...
            showAlert(`文件已下载: ${finalFilename}`, 'success');
        } catch (e) {
            if (e.name !== 'AbortError') {
    ...
    async function runAnalyzeV1() {
        ...
        try {
            ...
            const data = await Api.endpoints.xpl.analyzeV1({
