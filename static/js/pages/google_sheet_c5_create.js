// Google Sheet 创建任务页（C5）（templates/google_sheet_c5/create.html 的页面逻辑）。
// 批内收敛：同名函数已提升至 static/js/common/business/（Biz.*），见 F3 收敛 pass。
// 自 templates/google_sheet_c5/create.html 内联脚本原样抽离；接口调用经 common/api.js。

// ── 原 Jinja 服务端渲染点的前端等价实现（静态化，docs/design/frontend-refactor/03 §5）──
// 原 `url_for('google_sheet.index', version=request.args.get('version', 'c5'))`
const versionParam = new URLSearchParams(location.search).get('version');
if (versionParam && versionParam !== 'c5') {
    const backLink = document.querySelector('a[href="/google-sheet/?version=c5"]');
    if (backLink) {
        backLink.href = '/google-sheet/?version=' + encodeURIComponent(versionParam);
    }
}
    const GOOGLE_SHEET_TABLE_TYPE = 'c5';
    ...
    function loadTemplates() {
        // 只加载 C5 类型的模板（后端按 config.task_type 精确匹配，必须传 task_type= 查询串）
        Api.endpoints.template.list('task_type=google_sheet_C5')
            .then(data => {
    ...
    function bindEvents() {
        ...
        const inputFields = [
            ...
            'start_date', 'end_date', 'price_mode', 'kline_adjustment'
        ];
    ...
    function submitTask() {
        ...
        const priceMode = isCustomKline ? null : (document.getElementById('price_mode')?.value || 'vwap_price');
