// 创建页版本分发器（docs/design/frontend-refactor/03 §4.1）。
// /google-sheet/create?restart_task_id=…（无 version）由此页查任务补 version 后重定向；
...
(function () {
    ...
    const restartId = params.get('restart_task_id');
    if (!restartId) {
        // 无可解析参数：落回默认 C3 创建页（与原服务端缺省分支一致）。
        window.location.replace('/google-sheet/create');
        return;
    }

    Api.endpoints.task.detail(restartId).then(function (task) {
        const version = taskTypeToVersion(task && task.task_type);
        const sp = new URLSearchParams(window.location.search);
        if (version) {
            sp.set('version', version);
            window.location.replace('/google-sheet/create?' + sp.toString());
        } else {
            // 未知类型/任务：落回默认 C3 创建页（原服务端缺省分支）。
            window.location.replace('/google-sheet/create?' + sp.toString());
        }
    }).catch(function () {
        const sp = new URLSearchParams(window.location.search);
