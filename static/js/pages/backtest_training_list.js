...
function buildDetailHref(taskId) {
    ...
    return `/backtest-training/detail/${encodeURIComponent(taskId)}?${params.toString()}`;
}
...
function renderTaskCell(task) {
    ...
    return `
        ...
        <div class="task-meta text-body-secondary">ID: ${Biz.escapeHtml(shortId)} · Sheet: ${Biz.escapeHtml(sheetName)}</div>
    `;
}
...
// 执行参数去重：完全相同的参数行只展示一次；整行皆空的参数组跳过。
...
function selectAllExportableTasks() {
    ...
    getExportableTasks().forEach((task) => {
...
async function exportSelectedBatchTasks() {
    ...
    try {
        ...
        const filename = Biz.resolveDownloadFilename(
            ...
            'backtest_global_preview_batch.zip'
        );
