    ...
    function loadTaskResults(page) {
        ...
        Api.endpoints.task.results(currentTaskId, `page=${targetPage}&per_page=${resultsPerPage}`).then(function(data) {
            ...
            if (data && Array.isArray(rdata.items)) {
                ...
                resultsTotalCount   = rdata.total != null ? rdata.total : groupedResults.length;
                resultsTotalSuccess = typeof rdata.total_success === 'number' ? rdata.total_success : null;
                resultsTotalFailed  = typeof rdata.total_failed  === 'number' ? rdata.total_failed  : null;
