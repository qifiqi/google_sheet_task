...
(function () {
  ...
  function callPageHook(name, ...args) {
    const hook =
      window.Biz.formState && window.Biz.formState.hooks
  ...
  function syncSelectedTokenMeta() {
    ...
    const selected = googleSheetTokens.find(
      (token) => String(token.id) === String(tokenIdEl.value),
    );
  ...
  function applyPendingTokenSelection() {
    ...
    if (
      tokenId &&
      Array.from(tokenIdEl.options).some(
        (option) => option.value === String(tokenId),
      )
    ) {
      matchedValue = String(tokenId);
    } else if (tokenFile) {
      const matchedToken = googleSheetTokens.find(
        (token) => token.token_file === tokenFile,
      );
      if (matchedToken) {
    ...
    if (
      !matchedValue &&
      pendingTokenSelection.token_selection_mode === googleSheetRandomValue
    ) {
  ...
  function loadGoogleSheetTokens(preferredValue = null) {
    ...
    Api.endpoints.googleSheet
      .tokens("?task_type=google_sheet")
      .then((data) => {
  ...
  function importGoogleSheetToken() {
    ...
    Api.endpoints.googleSheet
      .importToken({ token_file: tokenFile })
      .then((data) => {
        showNotification(data.message || "Token导入成功", "success");
        pendingTokenSelection = {
          token_id: data.token && data.token.id ? String(data.token.id) : "",
        };
  ...
  function renderProductCodeChips() {
    ...
    productCodes.forEach(function (code) {
      ...
      span.className =
        "badge bg-primary text-white d-inline-flex align-items-center";
  ...
  function fillFormWithRestartTask(taskId) {
    Api.endpoints.task
      .detail(encodeURIComponent(taskId))
      .then((data) => {
  ...
  async function loadStockMarkets(selectedValue = "cn") {
    ...
    select.innerHTML = (payload?.stock_markets || [])
      .map(
        (market) => `<option value="${market.value}">${market.label}</option>`,
      )
      .join("");
