...
(function (Biz) {
  ...
  function normalizeBacktestResultPayload(payload) {
    ...
    if (
      ...
      typeof payload.calculate_metrics === "object"
    ) {
      const sheetResult =
        payload.sheet_result && typeof payload.sheet_result === "object"
          ? payload.sheet_result
          : Object.fromEntries(
              Object.entries(payload).filter(
                ([key]) => key !== "calculate_metrics",
              ),
            );
      return {
        ...payload.calculate_metrics,
  ...
  function fillScalarsTable(tbodyId, r) {
    ...
    scalarKeys.forEach((k) => {
      ...
      let v = r[k];
      if (
        k.includes("profit_annual") ||
        k.includes("outperform_year") ||
        k.includes("winning_rate")
      ) {
        v = fmtPct(v, 2);
  ...
  function fillSheetResultTable(tbodyId, sheetResult) {
    ...
    if (
      ...
      Array.isArray(sheetResult)
    ) {
      tbody.innerHTML =
        '<tr><td colspan="2" class="text-center text-body-secondary">暂无数据</td></tr>';
      return;
    }
  ...
  function fillMonthlyExcessReturnsTable(list, tbodyId) {
    ...
    if (!Array.isArray(list) || list.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="text-center text-body-secondary">暂无数据</td></tr>';
      return;
    }
  ...
  function fillExcessReturnsTable(r) {
    ...
    if (!Array.isArray(r.excess_returns)) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="text-center text-body-secondary">暂无数据</td></tr>';
      return;
    }
  ...
  function fillKamaMerged(indexList, startList, tbodyId) {
    ...
    const years = Array.from(new Set([...idx.keys(), ...st.keys()]))
      .filter((y) => y !== "all")
      .sort();

    if (years.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" class="text-center text-body-secondary">暂无数据</td></tr>';
      return;
    }
  ...
  function fillSharpeTable(obj, tbodyId) {
    ...
    if (tbody.children.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" class="text-center text-body-secondary">无可展示条目</td></tr>';
    }
  }
  ...
  function fillSharpeMerged(indexObj, startObj, tbodyId) {
    ...
    const keys = Array.from(
      new Set([...Object.keys(idx), ...Object.keys(st)]),
    ).sort();

    if (keys.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="11" class="text-center text-body-secondary">暂无数据</td></tr>';
      return;
    }
    ...
    keys.forEach((k) => {
      ...
      const s = st[k];
      const base =
        i && typeof i === "object" ? i : s && typeof s === "object" ? s : null;
      if (!base) return;
    ...
    if (tbody.children.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="11" class="text-center text-body-secondary">无可展示条目</td></tr>';
    }
  }
  ...
  function renderTabAnnualAndDrawdown(r) {
    ...
    const years = Array.from(new Set([...idxAnnual.keys(), ...stAnnual.keys()]))
      .filter((y) => y !== "all")
      .sort();
    ...
    const idxDd = idxByYear(
      r.index_maximum_drawdown?.year_maximum_drawdown,
      "drawdown",
    );
    const stDd = idxByYear(
      r.start_maximum_drawdown?.year_maximum_drawdown,
      "drawdown",
    );
    const ddYears = Array.from(new Set([...idxDd.keys(), ...stDd.keys()]))
      .filter((y) => y !== "all")
      .sort();
  ...
  function renderTabMonthlyExcessReturns(r) {
    fillMonthlyExcessReturnsTable(
      r.monthly_excess_returns,
      "tbl-monthly-excess-returns",
    );
  }
  ...
  function renderTabSotino(r) {
    fillSotinoMerged(
      r.index_sortino_ratio,
      r.start_sortino_ratio,
      "tbl-sotino",
    );
  }
  ...
  function renderTabSharpe(r) {
    fillSharpeMerged(
      r.index_sharpe_ratios,
      r.start_sharpe_ratios,
      "tbl-sharpe",
    );
  }
  ...
  function renderSummary(r) {
    document.getElementById("v1-outperform-year").textContent =
      r.outperform_year !== undefined ? fmtPct(r.outperform_year, 2) : "-";
    document.getElementById("v1-monthly-excess-vol").textContent =
      r.monthly_excess_volatility !== undefined
        ? fmtNum(r.monthly_excess_volatility, 4)
        : "-";
    document.getElementById("v1-excess-dd-win").textContent =
      r.excess_drawdown_winning_rate !== undefined
        ? fmtPct(r.excess_drawdown_winning_rate, 2)
        : "-";

    const allExcess = Array.isArray(r.excess_returns)
      ? r.excess_returns.find((x) => String(x.year) === "all")
      : null;
    document.getElementById("v1-excess-annual-all").textContent = allExcess
      ? fmtPct(allExcess.annualized_return_diff, 2)
      : "-";

    document.getElementById("v1-index-profit-annual").textContent =
      r.index_profit_annual !== undefined
        ? fmtPct(r.index_profit_annual, 2)
        : "-";
    document.getElementById("v1-start-profit-annual").textContent =
      r.start_profit_annual !== undefined
        ? fmtPct(r.start_profit_annual, 2)
        : "-";
    document.getElementById("v1-index-monthly-vol").textContent =
      r.index_monthly_return_volatility !== undefined
        ? fmtNum(r.index_monthly_return_volatility, 6)
        : "-";
    document.getElementById("v1-start-monthly-vol").textContent =
      r.start_monthly_return_volatility !== undefined
        ? fmtNum(r.start_monthly_return_volatility, 6)
        : "-";
  }
