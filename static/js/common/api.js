...
(function () {
  ...
  const endpoints = {
    ...
    task: {
      ...
      batchCreate: function (payload) {
        return post("/api/tasks/batch-create", payload);
      },
      ...
      logs: function (taskId) {
        return get("/api/tasks/" + taskId + "/logs");
      },
      results: function (taskId, query) {
        return get(
          "/api/tasks/" + taskId + "/results" + (query ? "?" + query : ""),
        );
      },
      statusCheck: function (taskId) {
        return get("/api/tasks/" + taskId + "/status-check");
      },
      restart: function (taskId, payload) {
        return post("/api/tasks/" + taskId + "/restart", payload);
      },
      createRestart: function (taskId, payload) {
        return post("/api/tasks/" + taskId + "/create-restart", payload);
      },
      updateConfig: function (taskId, payload) {
        return put("/api/tasks/" + taskId + "/config", payload);
      },
    },
    stock: {
    ...
    template: {
      list: function (query) {
        return get("/api/templates" + (query ? "?" + query : ""));
      },
      detail: function (templateId) {
        return get("/api/templates/" + templateId);
      },
      create: function (payload) {
        return post("/api/templates", payload);
      },
      update: function (templateId, payload) {
        return put("/api/templates/" + templateId, payload);
      },
    ...
    googleSheet: {
      ...
      importToken: function (payload) {
        return post("/api/google-sheet-tokens/import", payload);
      },
      // admin Token 管理页（admin/config）的新增入口：成功 toast 读服务端 message，
      // 返回完整信封（行为零变化），与上方去信封的 importToken（建单页表单用）并存。
      importTokenEnvelope: function (payload) {
        return post("/api/google-sheet-tokens/import", payload, {
          envelope: true,
        });
      },
      tokenDetail: function (tokenId) {
        return get(
      ...
      updateToken: function (tokenId, payload) {
        return put(
          "/api/google-sheet-tokens/" + encodeURIComponent(tokenId),
          payload,
        );
      },
      deleteToken: function (tokenId) {
        return del("/api/google-sheet-tokens/" + encodeURIComponent(tokenId));
      },
    },
    adminGoogleSheets: {
      // admin Google Sheet 管理页：成功 toast 读服务端 message，返回完整信封。
      create: function (payload) {
        return post("/api/google-sheets", payload, { envelope: true });
      },
      update: function (sheetId, payload) {
        return put("/api/google-sheets/" + sheetId, payload, {
          envelope: true,
        });
      },
      remove: function (sheetId) {
        return del("/api/google-sheets/" + sheetId, { envelope: true });
      },
    },
    adminNavigation: {
      // admin 路由管理页：成功 toast 读服务端 message，写操作返回完整信封。
      list: function () {
        return get("/api/navigation-menu-items");
      },
      create: function (payload) {
        return post("/api/navigation-menu-items", payload, { envelope: true });
      },
      update: function (itemId, payload) {
        return put(
          "/api/navigation-menu-items/" + encodeURIComponent(itemId),
          payload,
          { envelope: true },
        );
      },
      remove: function (itemId) {
        return del("/api/navigation-menu-items/" + encodeURIComponent(itemId), {
          envelope: true,
        });
      },
    },
    adminResults: {
      list: function (query) {
        return get("/api/results" + (query ? "?" + query : ""));
      },
      detail: function (resultId) {
        return get("/api/results/" + resultId);
      },
      remove: function (resultId) {
        return del("/api/results/" + resultId);
      },
    },
    adminModelSummary: {
    ...
    export: {
      ...
      modelSummaryCsv: function (query, options) {
        return fetch(
          "/api/exports/model-summary" + (query ? "?" + query : ""),
          options,
        );
      },
    },
