// 全局变量
    ...
    function parseJsonArrayInput(rawValue, label) {
        ...
        if (!Array.isArray(parsed)) {
            throw new Error(`${label} 必须是数组格式`);
        }

        return parsed;
    }

    function getCurrentTaskConfigObject() {
        if (!currentTaskData || !currentTaskData.config) {
            return {};
        }

        // 详情页和创建页共用同一套配置解析规则，避免编辑弹窗与创建页行为不一致。
        return parseTaskConfigObject(currentTaskData.config);
    }
    ...
    function renderEditSheetsRows(sheets) {
        ...
        data.forEach(s => {
            ...
            row.innerHTML = `
                <div class="col-md-4">
                    <label class="form-label small mb-1">spreadsheet_id</label>
                    <input type="text" class="form-control form-control-sm edit-sheet-spreadsheet-id" value="${escapeHtmlAttr(s.spreadsheet_id || '')}">
                </div>
                <div class="col-md-4">
                    <label class="form-label small mb-1">title</label>
                    <input type="text" class="form-control form-control-sm edit-sheet-title" value="${escapeHtmlAttr(s.title || '')}">
                </div>
                <div class="col-md-4">
                    <label class="form-label small mb-1">sheet_name</label>
                    <input type="text" class="form-control form-control-sm edit-sheet-name" value="${escapeHtmlAttr(s.sheet_name || '')}">
                </div>
            `;
            list.appendChild(row);
        });
    ...
    function openEditConfigModal() {
        ...
        const config = getCurrentTaskConfigObject();
        ...
        const priceModeEl = document.getElementById('edit-price-mode');
        if (priceModeEl) {
            priceModeEl.value = config.price_mode || 'kp_price';
        }
        ...
        if (tokenTypeSelect) {
            ...
            if (!tokenTypeSelect._bindedForToggle) {
                tokenTypeSelect.addEventListener('change', function () {
                    ...
                    if (fileContainer) fileContainer.style.display = t === 'file' ? 'block' : 'none';
                    if (jsonContainer) jsonContainer.style.display = t === 'json' ? 'block' : 'none';
                });
                tokenTypeSelect._bindedForToggle = true;
            }
        }

        if (fileContainer) fileContainer.style.display = tokenType === 'file' ? 'block' : 'none';
        if (jsonContainer) jsonContainer.style.display = tokenType === 'json' ? 'block' : 'none';
        ...
        const editParam2El = document.getElementById('edit-param2');
        if (editParam2El) {
            editParam2El.value = Array.isArray(config.parameters && config.parameters[1])
                ? JSON.stringify(config.parameters[1], null, 2)
                : '';
        }
        const editParam3El = document.getElementById('edit-param3');
        if (editParam3El) {
            editParam3El.value = Array.isArray(config.parameters && config.parameters[2])
                ? JSON.stringify(config.parameters[2], null, 2)
                : '';
        }
    ...
    function loadTaskConfig(config) {
        ...
        const sheets = Array.isArray(config.sheets) ? config.sheets : [];
        const codeCount = Array.isArray(config.parameters && config.parameters[0])
                ? config.parameters[0].length
                : 0;
        const dateRange = (config.start_date || '-') + ' ~ ' + (config.end_date || '-');
        let dateRangeModeDisplay = '-';
        if (Array.isArray(config.date_range_mode)) {
            dateRangeModeDisplay = config.date_range_mode.join(',');
        } else if (typeof config.date_range_mode === 'string' && config.date_range_mode) {
            dateRangeModeDisplay = config.date_range_mode;
        }
        const dateRangeMode = config.date_range_mode || 'full';

        // 工作表列表：每个表一行（表N + ID + 名称）
        const sheetsBlock = document.createElement('div');
        sheetsBlock.className = 'mb-2';
        sheetsBlock.innerHTML = '<strong class="d-block mb-1">工作表</strong>';

        if (sheets.length > 0) {
            sheets.forEach((s, idx) => {
                const row = document.createElement('div');
                row.className = 'small text-muted mb-1';
                const titleText = s.title || '';
                row.innerHTML = `
                    <span class="badge bg-secondary me-2">表${idx + 1}</span>
                    <span>Spreadsheet ID: ${s.spreadsheet_id || '-'}</span>
                    <span class="ms-2">Sheet: ${s.sheet_name || '-'}</span>
                    <span class="ms-2 text-muted">Title: ${titleText || '-'}</span>
                `;
        ...
        const configItems = [
            {label: '产品代码数', value: codeCount, icon: 'bi-list-ul'},
            {label: '统计方式', value: config.count_mode || 'total', icon: 'bi-calculator'},
            {label: '使用价格', value: config.price_mode || 'sp_price', icon: 'bi-calculator'},
    ...
    function groupResults(resultsArray) {
        ...
        resultsArray.forEach((item) => {
            ...
            Object.keys(resultObj).forEach((keyName) => {
                ...
                const d2 = metrics.D2 != null ? metrics.D2 : null;
                const d3 = metrics.D3 != null ? metrics.D3 : null;
                const d4 = metrics.D4 != null ? metrics.D4 : null;
                const d5 = metrics.D5 != null ? metrics.D5 : null;
                const d6 = metrics.D6 != null ? metrics.D6 : null;
                const d7 = metrics.D7 != null ? metrics.D7 : null;

                const startReturn = metrics.start_return_xpl || {};
                const indexReturn = metrics.index_return_xpl || {};

                const startSharpe = startReturn.sharpe_ratio != null ? startReturn.sharpe_ratio : null;
                const indexSharpe = indexReturn.sharpe_ratio != null ? indexReturn.sharpe_ratio : null;
        ...
        if (!Array.isArray(pageResults) || pageResults.length === 0) {
            ...
            pageResults.forEach((group, idx) => {
                ...
                const modelsRows = (group.models || []).map((model, mIndex) => {
                    const d2Text = model.d2 != null
                            ? (typeof model.d2 === 'number' ? model.d2.toFixed(4) : String(model.d2))
                            : '-';
                    const d3Text = model.d3 != null
                            ? (typeof model.d3 === 'number' ? model.d3.toFixed(4) : String(model.d3))
                            : '-';
                    const d4Text = model.d4 != null
                            ? (typeof model.d4 === 'number' ? model.d4.toFixed(4) : String(model.d4))
                            : '-';
                    const d5Text = model.d5 != null
                            ? (typeof model.d5 === 'number' ? model.d5.toFixed(4) : String(model.d5))
                            : '-';
                    const d6Text = model.d6 != null
                            ? (typeof model.d6 === 'number' ? model.d6.toFixed(4) : String(model.d6))
                            : '-';
                    const d7Text = model.d7 != null
                            ? (typeof model.d7 === 'number' ? model.d7.toFixed(4) : String(model.d7))
                            : '-';
                    let iXplText = '-';
                    if (model.indexSharpe != null) {
                        const v = Number(model.indexSharpe);
                        iXplText = isNaN(v) ? String(model.indexSharpe) : v.toFixed(4);
                    }

                    let sXplText = '-';
                    if (model.startSharpe != null) {
                        const v2 = Number(model.startSharpe);
                        sXplText = isNaN(v2) ? String(model.startSharpe) : v2.toFixed(4);
                    }
    ...
    function changeResultsPage(page) {
        ...
        function exportResultsToCSV() {
            ...
            ajaxRequest(`/api/tasks/${currentTaskId}/results`, 'GET', null, function (err, data) {
                ...
                const exportGroupedResults = groupResults(data.results);
                ...
                exportGroupedResults.forEach(group => {
                    const baseInfo = {
                        ...
                        a1: group.a1 != null ? group.a1 : '',
                        b1: group.b1 != null ? group.b1 : '',
                    ...
                    if (models.length === 0) {
                        const emptyMetrics = metricKeys.map(() => '');
                        rows.push([
                            baseInfo.stepIndex,   // step_index
                            baseInfo.success,     // success
                            baseInfo.stockCode,   // stock_code
                            baseInfo.klineRange,  // kline_range
                            baseInfo.a1,          // xm
                            baseInfo.b1,          // ml
                            '',                   // model_title
                            ...emptyMetrics,      // D2-D20 指标列
                            '',                   // start_avg_monthly_return
                            '',                   // start_monthly_std_dev
                            '',                   // start_annual_std_dev
                            '',                   // start_xpl
                            ...
                            '',                   // index_annual_std_dev
                            '',                   // index_xpl
                            baseInfo.taskId,      // task_id
                            baseInfo.timestamp,   // timestamp
                            baseInfo.errorMessage // error_message
                        ]);
                        return;
                    }

                    models.forEach(model => {
                        const iSharpe = model.indexSharpe != null ? model.indexSharpe : '';
                        const sSharpe = model.startSharpe != null ? model.startSharpe : '';

                        // 从 rawMetrics 中读取 start/index 的 xpl 统计字段
                        ...
                        // D2-D20：按 metricKeys 顺序从 rawMetrics 中取值
                        const metricValues = metricKeys.map(key => (
                                metrics[key] != null ? metrics[key] : ''
                        ));
                ...
                if (rows.length > 1) {
                    ...
                    // stock_code 与 kline_range 的固定列索引
                    const STOCK_CODE_COL = 2; // stock_code
                    const KLINE_RANGE_COL = 3; // kline_range

                    // 找到 Return% 在 metricKeys 中对应的列索引（D2 -> Return%）
                    const returnMetricIndex = metricKeys.indexOf('D2');
                    let RETURN_COL = null;
                    if (returnMetricIndex !== -1) {
                        // 前面 7 列是固定列，从第 8 列开始是指标列
                        RETURN_COL = 7 + returnMetricIndex;
                    }

                    const parseEndYear = (range) => {
                        if (!range) return -Infinity;
                        const str = String(range);
                        const parts = str.split('~');
                        if (parts.length < 2) return -Infinity;
                        const endPart = parts[1].trim();
                        const yearStr = endPart.slice(0, 4);
                        const year = parseInt(yearStr, 10);
                        return Number.isNaN(year) ? -Infinity : year;
                    };

                    const parseReturn = (val) => {
                        if (val == null || RETURN_COL === null) return -Infinity;
                        const num = parseFloat(String(val).replace('%', '').trim());
                        return Number.isNaN(num) ? -Infinity : num;
                    };

                    dataRows.sort((a, b) => {
                        const stockA = (a[STOCK_CODE_COL] || '').toString();
                        const stockB = (b[STOCK_CODE_COL] || '').toString();
                        if (stockA !== stockB) {
                            // stock_code 降序
                            return stockB.localeCompare(stockA);
                        }

                        const yearA = parseEndYear(a[KLINE_RANGE_COL]);
                        const yearB = parseEndYear(b[KLINE_RANGE_COL]);
                        if (yearA !== yearB) {
                            // 结束年份降序
                            return yearB - yearA;
                        }
                        ...
                        if (RETURN_COL !== null) {
                            const retA = parseReturn(a[RETURN_COL]);
                            const retB = parseReturn(b[RETURN_COL]);
                            if (retA !== retB) {
                                // Return% 降序
                                return retB - retA;
                            }
        ...
        function showResultDetail(groupIndex, modelIndex) {
            ...
            const startReturn = metrics.start_return_xpl || null;
            const indexReturn = metrics.index_return_xpl || null;
            ...
            if (startReturn) {
                ...
                <div class="col-md-6 mb-2">
                    <div class="card border-secondary">
                        <div class="card-header py-1 px-2 small fw-semibold">start return xpl</div>
                        <div class="card-body py-2 px-2 small">
                            ...
                            ${renderStatLine('平均月收益', startReturn.avg_monthly_return.toFixed(6))}
                            ${renderStatLine('月度波动率', startReturn.monthly_std_dev.toFixed(6))}
                            ${renderStatLine('年化波动率', startReturn.annual_std_dev.toFixed(6))}
                            ${renderStatLine('夏普率', startReturn.sharpe_ratio.toFixed(6))}
                        </div>
                    </div>
                </div>
            `);
            ...
            if (indexReturn) {
                ...
                <div class="col-md-6 mb-2">
                    <div class="card border-secondary">
                        <div class="card-header py-1 px-2 small fw-semibold">index return xpl</div>
                        <div class="card-body py-2 px-2 small">
                            ...
                            ${renderStatLine('平均月收益', indexReturn.avg_monthly_return.toFixed(6))}
                            ${renderStatLine('月度波动率', indexReturn.monthly_std_dev.toFixed(6))}
                            ${renderStatLine('年化波动率', indexReturn.annual_std_dev.toFixed(6))}
                            ${renderStatLine('夏普率', indexReturn.sharpe_ratio.toFixed(6))}
                        </div>
                    </div>
                </div>
            `);
            ...
            const metricKeys = Object.keys(metrics).filter(k => k !== 'start_return_xpl' && k !== 'index_return_xpl');
            ...
            for (let i = 0; i < keys.length; i += 2) {
                ...
                const v1 = metrics[k1];
                const v2 = k2 != null ? metrics[k2] : undefined;
        ...
        function formatMetricDisplayValue(value) {
            ...
            if (typeof value === 'object') {
                // 常规对象在表格里显示为 JSON（特殊的 start_return_xpl / index_return_xpl 已在上方卡片中处理，不会走到这里）
        ...
        function goToCreateRestartTask() {
            window.location.href = `/google-sheet/create?version=c5&restart_task_id=${encodeURIComponent(currentTaskId || '')}`;
        }
            ...
            ajaxRequest(`/api/tasks/${currentTaskId}/create-restart`, 'POST', {}, function (err, data) {
                if (data && data.status === 'success') {
                    ...
                    if (confirm('是否跳转到新任务的详情页面？')) {
                        window.location.href = `/google-sheet/detail?task_id=${data.new_task_id}&version=c5`;
                    }
                } else {
                    const errorMessage = (data && data.message) ? data.message : (err ? err.message : '未知错误');
        ...
        function saveTaskConfig() {
            try {
                ...
                const list = document.getElementById('edit-sheets-list');
                const sheets = [];
                const baseConfig = Object.assign({}, getCurrentTaskConfigObject());
                if (list) {
                    list.querySelectorAll('.edit-sheet-row').forEach(row => {
                        ...
                        if (sid || sname || title) {
                            if (!sid || !sname) {
                                throw new Error('每一行 Sheets 配置都必须填写 spreadsheet_id 和 sheet_name');
                            }
                            const item = {
                                spreadsheet_id: sid,
                                sheet_name: sname
                ...
                if (sheets.length === 0) {
                    throw new Error('至少需要保留 1 组 Sheet 配置');
                }

                // 其它参数
                const countMode = document.getElementById('edit-count-mode').value || 'n_plus_1';
                const priceMode = document.getElementById('edit-price-mode').value || 'kp_price';
                const fullModeEl = document.getElementById('edit-date-range-full');
                const recentModeEl = document.getElementById('edit-date-range-recent');
                ...
                const param2 = parseJsonArrayInput(document.getElementById('edit-param2').value, '参数2');
                const param3 = parseJsonArrayInput(document.getElementById('edit-param3').value, '参数3');
                ...
                const config = {
                    ...baseConfig,
                    task_type: baseConfig.task_type || 'google_sheet_C5',
                    ...
                    token_file: tokenType === 'file' ? tokenFile : '',
                    token_json: tokenType === 'json' ? tokenJson : ''
                };
