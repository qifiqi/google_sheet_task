...
| 2026-09-05 | R4（commit 5a39e67） | 通过 | scheduler_api(14 点) 去直连：CRUD+调度同步编排全部下沉 scheduler_service（stats/分页响应结构、create/update/delete/toggle、run_task_now、执行状态聚合、get_required_task 前置检查）。偏差：① cron/params 校验自路由下沉并 BadRequestError→ValidationError（400+文案不变）；② update 端点保留"存在性检查先于 parse_body"的原顺序语义（路由先调 get_required_task 再 parse，双故障场景 404 优先于 400）；③ toggle 的 is_active 缺省取反逻辑原样保留（data.get('is_active', not task.is_active)，显式 null 行为不变）。验证：grep 残留为空；489 passed / 3 豁免 / 10 skipped 与基线一致。 |
| 2026-09-05 | 审计批 C | 通过 | model_summary_service（2017 行）四拆为 app/services/model_summary/ 包：extractor（SummaryRecord+全部纯函数与领域常量，lookup_stock_metadata/xpl_analyzer/upgrade_historical_metrics 随迁）、query（SummaryQueryMixin：query/_query_all_results/_summary_from_items/_columns_for_task_type/_empty_response/_record_to_dict）、exporter（SummaryExportMixin：export_csv/_render_csv + CSV_LEADING/TRAILING + _csv_text 系）、jobs（SummaryJobMixin：差分 upsert/rebuild/后台作业注册表/TaskLog 写入）、facade（mixin 组合 + _jobs/_jobs_lock/_index_lock 实例状态 + 单例）。原 model_summary_service.py 缩为 5 符号 re-export 门面（对齐 task/facade 先例）：admin_api/export_service/google_sheet_service_base/两测试文件 import 零改动。信封收敛：admin_api /api/model-summary 由 jsonify 透传改 success(data=payload)（业务键整体移入 data，键名不变）；_query_all_results 缺 stock_code 的 {"status":"error"} 返回改抛 ValidationError（400 信封）；templates/admin/model_summary.html 查询 fetch 处理改读 data.data.*（rebuild/status 块本已读 data.data.job 不动）；export_csv 翻页循环删除永假的 status!=\"success\" 分支（错误路径由异常承载），载荷内 status 业务键保留。测试同批更新：xpl_analyzer monkeypatch 目标改 model_summary.extractor；缺股票断言改 pytest.raises(ValidationError)。偏差：① 服务载荷内 "status":"success" 业务键保留（消费方/测试零改动，随 data 下发）；② export_service 的 status 防御检查保留（现永不触发，守卫未来语义）；③ extractor 存量未用局部变量随迁未清。注：全量门禁期间工作区混入并发会话（R 批次路由收编）未提交变更，本批提交按明确路径隔离。验证：489 passed / 3 豁免 / 10 skipped / 0 error（间歇 .pytest_tmp WinError32 文件锁抖动为环境级，隔离复跑通过）。 |
| 2026-09-05 | 审计批 D | 通过 | repository 命名治理（机械改名 35 处，1:1 无语义变化）：find_summary_index_entities_by_result(_ids/_task_ordered)→list_summary_index_entities_*（3）、product_cache_exists→exists_product_cache、page_with_statistics→list_paginated_with_statistics、recent→list_recent、distinct_task_types→list_distinct_task_types、find_due→list_due、find_by_name_and_function→get_by_name_and_function、stats→get_stats、find_first_by_spreadsheet→get_by_spreadsheet_id、find_duplicate→get_duplicate_row、task_log_repository.add→create_log、find_by_context→get_by_context；调用点同步（服务 11 文件 + test_repositories.py 6 处）。AGENTS.md 命名约定补录领域动词前缀：upsert_/acquire_/release_/occupy/mark_/revert_/record_/dedupe_/refresh_entity/sum_/apply_；数据访问章节终验声明更新为审计批 A~D 复验。偏差：① task_result_repository（latest_time_by_task）、rbac_repository（username_exists/role_code_exists）、base.add_entity 三处改名排除——并发会话（R 批次路由收编）正在这些文件上有未提交变更，避免提交混叠，登记为待办；② occupancy.py 存量死 import json 顺手清理；③ 执行记录表存在两个"审计批 C"行（本会话 model_summary 四拆 与并发会话 执行链落库收编 撞名，内容可区分，保留不重写）。验证：旧名 grep 归零；489 passed / 3 豁免 / 10 skipped。 |
| 2026-09-05 | R5（commit d277cd4） | 通过 | auth_api(30 点) 去直连：新建 rbac_service（模块函数形态），认证态（login_user/refresh_tokens/logout_user/change_password）+ 用户/角色/权限编排全部下沉；token 签发仍经 app.utils.auth（B5 契约：缓存留 auth 层）；删用户同事务原子性原样保持；is_alert_oncall 后置计算随编排迁移。偏差：① 凭证/权限类失败由路由 error() 信封改抛 ValidationError/UnauthorizedError——HTTP 状态码（400/401）与文案逐条对齐不变；② _can_alert_oncall/_role_code/DEV_ROLE_CODES 自路由迁入服务（ding_talk_notifier 自有同名常量独立保留，范围外）；③ auth_api 309→109 行。验证：grep 残留为空；冒烟 149 路由不变；489 passed / 3 豁免 / 10 skipped 与基线一致。 |
| 2026-09-05 | R 系列终验（AGENTS.md 更新） | 通过 | 全部 18 个路由文件 repository import/调用清零；routes 内 ORM grep 为空、手写 error 信封为空；路由 149 条全程未变；每批全量 pytest 与基线一致（489 passed / 3 豁免 / 10 skipped）。AGENTS.md"数据层分层规则"与"数据访问"章节同步：routes 禁止直接 import/调用 repository 生效，读路径归宿索引（task 门面/task_template_service/navigation_service/rbac_service/config_manager）。新增服务：TaskTemplateService、navigation_service、rbac_service、update_task_ratios；task 门面只读入口 get_task/get_required_task/get_required_task_entity/get_task_result/resolve_result_task_id/get_empty_tasks_page/get_distinct_task_types、结果入口 get_results_paginated/get_result_detail/delete_result/get_task_results_page_raw/get_required_result_entity/get_return_entity、runtime_view.get_runtime_detail、dashboard_query.get_dashboard_counts/get_recent_tasks、token_service.delete_token、config_manager.get_db_config_rows/update_config_row、scheduler_service 定时任务 CRUD 编排。 |
| 2026-09-05 | 结果持久化收敛（任务结果保存与重试逻辑） | 通过 | 6 个执行服务的重复 `_save_task_result` 收敛到 `google_sheet_service_base.py` 唯一定义（全库 1 处），底层新增 `task_result_repository.create_with_return(result_fields, return_fields=None, commit=True)`：TaskResult + 可选 TaskResultReturn 同事务、flush 回链 return_series_id、异常 rollback 后裸 raise。偏差与定论：① `create_with_return` 是对 02 §2.2 契约的扩展，契约来源为 6 个执行服务既有调用点的内联写入序列（构造→add→flush→回链→commit），行为逐点保持；② db_retry 不改：`safe_db_operation` 整操作重试链中，写方法异常已 rollback 后裸 raise（base 契约），重试从干净 session 开始，"flush 失败重试挂脏 session"问题由仓储层契约消除；`_save_task_result` 外层兜底 `task_result_repository.rollback()` 为 base.py:68 公共方法，合法；③ 服务差异以钩子保留：C3~C7 默认 `extract_return_rows(result)` 提取收益行，两个 backtest 服务显式 `return_date` 入参（缺有效日期抛 ValueError），multi_product 载荷 `_return_date` 键经 `_prepare_result_for_persistence` 剥离 + return_date 覆盖行来源，数据加工（normalize/sanitize/payload 钩子）留 service/base 层；④ backtest_multi_product_service.py 的对应改动已随并行审计批 B/D 落库，本批提交其余 7 文件；⑤ 6 个服务删除不再使用的 TaskResult/TaskResultReturn 模型、build_return_series_fields/extract_return_rows、db_retry_manager 导入（grep 零残留；被删符号无剩余使用点）。验证：py_compile 7 文件通过；定向回归 test_backtest_services + test_backtest_training_result_storage 25 passed；全量 489 passed / 3 基线豁免 / 10 skipped 与基线一致。 |
