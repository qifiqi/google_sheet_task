...
### 2.1 会话优先级：本地已有会话优先（2026-09-18 确认）

登录页落地时（`static/js/template-auth.js::bindLoginPage`）的判定顺序：

1. 本地存在 access token 且 `GET /api/auth/me` 成功（含 401 自动 refresh 续期）
   → 直接复用本地会话跳转 `next`，**不再换票**；fragment 里的 sso_token 读完即弃。
2. 本地无 token，或本地会话已彻底失效（refresh 也失败）→ 才用 sso_token 换票；
   换票失败展示错误并保留账号密码登录兜底。

口径来源：本服务同时是长时运行任务平台，本地会话承载"正在看的任务/页面权限"，
优先于入口身份断言，避免子服务刷新页面或二次点击主服务链接时被重复换票打断
（旧行为下，携带过期 sso_token 进入会顶掉一个原本可用的本地会话）。

已知后果（已接受）：同一浏览器先后被不同主服务账号使用时，第 1 步会沿用上一次
换票得到的本地身份。需要切换身份时，必须先在子服务退出登录（清 localStorage +
`gsc_access_token` cookie）再进入。
