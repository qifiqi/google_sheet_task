<template>
  <div class="backtest-create-page">
    <PageToolbar eyebrow="回测训练" title="创建回测任务" />
      ...
      >
        ...
        <el-form-item label="关联股票" prop="stock_code">
          <el-autocomplete
            ...
            :placeholder="stockSearch.loading.value ? '搜索中...' : '输入股票代码或名称'"
            :disabled="stockSearch.loading.value"
            value-key="code"
            clearable
        ...
        <el-form-item>
          <div class="form-actions">
            <el-button @click="router.push('/backtest/list')">取消</el-button>
...
import { importExcel } from '@/api/backtest'
...
const stockSearch = useStockSearch({ delay: 300 })

function handleStockQuery(query, cb) {
  stockSearch.search(query)
  // Use a short interval to check results since search is async
  const check = () => {
    if (!stockSearch.loading.value) {
      cb(stockSearch.results.value.map(item => ({
...
async function handleSubmit() {
  ...
  try {
    ...
    ElMessage.success('回测任务创建成功，即将跳转')
