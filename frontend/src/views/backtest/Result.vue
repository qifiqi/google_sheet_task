<template>
  <div class="backtest-result-page" v-loading="loading">
    <PageToolbar eyebrow="回测结果" :title="resultTitle">
      <template #actions>
...
import { getTaskResult, exportGlobalPreview } from '@/api/backtest'
...
const resultTitle = computed(() => result.value?.task_name || `回测结果 #${taskId.value}`)
...
async function handleExport() {
  ...
  try {
    ...
    a.href = url
    a.download = `backtest_result_${taskId.value}.xlsx`
    a.click()
