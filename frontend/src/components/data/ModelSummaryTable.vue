...
<template>
  <el-table v-loading="loading" :data="tableItems" empty-text="暂无汇总数据" class="model-summary-table">
    ...
    <el-table-column label="任务" min-width="190"><template #default="{ row }"><div class="model-summary-table__task"><strong>{{ row.task_name || '-' }}</strong><span>{{ taskTypeText(row.task_type) }} · {{ row.model_name || row.model_key || '-' }}</span></div></template></el-table-column>
