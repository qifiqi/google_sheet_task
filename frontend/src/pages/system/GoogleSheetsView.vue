...
const filteredSheets = computed(() => {
  ...
  return registry.sheets.value.filter((item) => (!tableType.value || item.table_type === tableType.value) && (!query || `${item.name} ${item.spreadsheet_id} ${item.remark || ''}`.toLowerCase().includes(query)))
})
...
async function save() {
  ...
  saving.value = true
  try {
    await registry.save({ id: editing.value?.id, name: form.name.trim(), spreadsheet_id: form.spreadsheet_id.trim(), table_type: form.table_type, remark: form.remark.trim(), is_active: form.is_active })
...
<template>
  <section class="google-sheets-page">
    ...
    <section class="google-sheets-page__panel">
      <div class="google-sheets-page__toolbar"><el-input v-model="keyword" clearable placeholder="搜索名称、ID 或备注" /><el-select v-model="tableType" clearable placeholder="全部类型"><el-option v-for="item in tableTypes" :key="item.value" :label="item.label" :value="item.value" /></el-select><span>共 {{ filteredSheets.length }} 条</span></div>
      ...
      <el-table v-loading="registry.loading.value" :data="filteredSheets" empty-text="暂无 Google Sheet">
        ...
        <el-table-column v-if="canManage()" fixed="right" label="操作" width="128"><template #default="{ row }"><el-button link type="primary" @click="openEdit(row)">编辑</el-button><el-button link type="danger" :disabled="row.is_in_use" @click="remove(row)">删除</el-button></template></el-table-column>
      </el-table>
    </section>
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑 Google Sheet' : '新增 Google Sheet'" width="min(640px, calc(100vw - 32px))" :close-on-click-modal="false">
      <el-form label-position="top" @submit.prevent="save"><div class="google-sheets-page__form-grid"><el-form-item label="名称" required><el-input v-model="form.name" maxlength="255" /></el-form-item><el-form-item label="表类型" required><el-select v-model="form.table_type"><el-option v-for="item in tableTypes" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item></div><el-form-item label="Spreadsheet ID" required><el-input v-model="form.spreadsheet_id" /></el-form-item><el-form-item label="备注"><el-input v-model="form.remark" type="textarea" :rows="3" maxlength="500" show-word-limit /></el-form-item><el-form-item><el-switch v-model="form.is_active" active-text="启用" inactive-text="停用" /></el-form-item></el-form>
