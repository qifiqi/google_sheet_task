...
import C31BatchResultDialog from '../../components/google-sheet/C31BatchResultDialog.vue'
...
const titleGroups = computed(() => {
  ...
  for (const sheet of draft.sheets) { const parsed = parseC31SheetTitle(sheet.title); if (parsed) groups.set(parsed.year, [...(groups.get(parsed.year) || []), sheet]) }
  return groups
})
...
function applyConfig(config: Record<string, unknown>, name = '', description = '') {
  draft.taskName = name || String(config.base_task_name || ''); draft.description = description || String(config.task_description || '')
  draft.stockCode = String((Array.isArray(config.stock_codes) ? config.stock_codes[0] : config.stock_code) || '')
  draft.marketType = config.market_type === 'en' ? 'en' : 'cn'; draft.priceMode = ['kp_price', 'sp_price'].includes(String(config.price_mode)) ? config.price_mode as C31CreateDraft['priceMode'] : 'vwap_price'; draft.klineAdjustment = ['back', 'none'].includes(String(config.kline_adjustment)) ? config.kline_adjustment as C31CreateDraft['klineAdjustment'] : 'forward'; draft.endDate = String(config.end_date || '')
  ...
  draft.sheets = sheets.length ? sheets.map((item) => { const sheet = item as Record<string, unknown>; return { spreadsheetId: String(sheet.spreadsheet_id || ''), title: String(sheet.title || ''), sheetName: String(sheet.sheet_name || '') } }) : [createEmptySheet()]
  const parameters = Array.isArray(config.parameters) ? config.parameters : []; draft.parameters = Array.from({ length: 6 }, (_, index) => parameters[index] ? JSON.stringify(parameters[index]) : '')
}
async function loadTemplates() { loadingTemplates.value = true; try { const payload = await requestJson<{ status: string; templates: TaskTemplate[] }>('/api/templates'); templates.value = (payload.templates || []).filter((item) => String(item.config.task_type || '').toLowerCase() === 'google_sheet_c31') } catch (error) { ElMessage.error(error instanceof Error ? error.message : '加载模板失败') } finally { loadingTemplates.value = false } }
async function loadTemplate(id: number | null) { if (!id) return; try { const template = await requestJson<TaskTemplate>(`/api/templates/${id}`); applyConfig(template.config, template.name, template.description || ''); ElMessage.success('模板已应用') } catch (error) { ElMessage.error(error instanceof Error ? error.message : '加载模板失败') } }
...
<template>
  <section class="c31-page">
    ...
    <el-form class="c31-page__form" label-position="top" @submit.prevent>
      <section class="c31-page__section"><header class="c31-page__section-header"><div><h2>批次信息</h2><p>一个批次会按股票、参数组合及年份组拆分为可独立执行的 C3 子任务。</p></div></header><div class="c31-page__meta-grid"><el-form-item label="任务模板"><el-select v-model="selectedTemplateId" clearable filterable :loading="loadingTemplates" placeholder="不使用模板" @change="loadTemplate"><el-option v-for="template in templates" :key="template.id" :label="template.name" :value="template.id" /></el-select></el-form-item><el-form-item label="任务 Base Name" required><el-input v-model="draft.taskName" placeholder="例如：strategy_batch" /></el-form-item><el-form-item label="股票代码" required><el-input v-model="draft.stockCode" placeholder="例如：601727" /></el-form-item><el-form-item label="市场"><el-radio-group v-model="draft.marketType"><el-radio-button value="cn">A股</el-radio-button><el-radio-button value="en">美股</el-radio-button></el-radio-group></el-form-item><el-form-item label="价格类型"><el-select v-model="draft.priceMode"><el-option label="加权平均价" value="vwap_price" /><el-option label="开盘价" value="kp_price" /><el-option label="收盘价" value="sp_price" /></el-select></el-form-item><el-form-item label="K 线复权"><el-select v-model="draft.klineAdjustment"><el-option label="前复权" value="forward" /><el-option label="后复权" value="back" /><el-option label="不复权" value="none" /></el-select></el-form-item><el-form-item label="结束日期"><el-date-picker v-model="draft.endDate" type="date" value-format="YYYY-MM-DD" placeholder="按后端默认逻辑" /></el-form-item><el-form-item class="c31-page__description" label="任务描述"><el-input v-model="draft.description" type="textarea" :autosize="{ minRows: 1, maxRows: 3 }" placeholder="可选，记录这批任务的用途" /></el-form-item></div></section>
      <section class="c31-page__section"><header class="c31-page__section-header"><div><h2>Sheet 对齐矩阵</h2><p>同一年份组内的 Sheet 数必须等于参数组合数，标题格式为“任意前缀-数字y-数字]”。</p></div><el-button :icon="CirclePlus" @click="addSheet">添加 Sheet</el-button></header><div class="c31-page__sheets"><GoogleSheetPicker v-for="(_sheet, index) in draft.sheets" :key="index" v-model="draft.sheets[index]" :index="index" :removable="draft.sheets.length > 1" :proxy-url="draft.execution.proxyUrl" @remove="removeSheet(index)" /></div><el-collapse v-model="expandedConfigSections" class="c31-page__execution-collapse"><el-collapse-item name="execution"><template #title><div class="c31-page__execution-collapse-title"><el-icon><Setting /></el-icon><div><strong>执行配置</strong><span>Token、认证方式和代理</span></div></div></template><ExecutionSettings v-model="draft.execution" :show-header="false" /></el-collapse-item></el-collapse></section>
