...
function applyConfig(config: Record<string, unknown>, name = '', description = '') {
  ...
  setSheets(config.sheets || [{
    ...
    title: config.title || config.spreadsheet_title,
    sheet_name: config.sheet_name,
  }])
  draft.execution = {
    tokenType: config.token_type === 'json' ? 'json' : 'file',
    tokenId: String(config.token_id || ''),
    tokenFile: String(config.token_file || ''),
    tokenJson: String(config.token_json || ''),
    proxyUrl: String(config.proxy_url || ''),
  }
  const values = Array.isArray(config.parameters) && Array.isArray(config.parameters[0]) ? config.parameters[0] : []
  draft.productCodes = values.length ? JSON.stringify(values) : ''
  ...
  draft.dateRangeMode = Array.isArray(config.date_range_mode)
    ? config.date_range_mode.filter((value): value is 'full' | 'recent' => value === 'full' || value === 'recent')
    : []
...
async function loadTemplates() {
  ...
  try {
    ...
    templates.value = (payload.templates || []).filter((item) => String(item.config.task_type || '').toLowerCase() === 'google_sheet_c4')
  } catch (error) {
...
async function submit() {
  try {
    ...
    window.setTimeout(() => { void router.push({ name: 'C4TaskDetail', params: { taskId: response.task_id } }) }, 500)
  } catch (error) {
...
<template>
  <section class="c4-create-page">
    ...
    <el-form class="c4-create-page__form" label-position="top" @submit.prevent>
      <section class="c4-create-page__section">
        ...
        <div class="c4-create-page__meta-grid">
          <el-form-item label="任务模板">
            <el-select v-model="selectedTemplateId" clearable filterable :loading="loadingTemplates" placeholder="不使用模板" @change="loadTemplate">
      ...
      <section class="c4-create-page__section">
        <header class="c4-create-page__section-header">
          <div><h2>Google Sheet 配置</h2><p>每个 Sheet 会按相同的产品代码批量执行。</p></div>
        ...
        <el-collapse v-model="expandedConfigSections" class="c4-create-page__execution-collapse">
          <el-collapse-item name="execution">
            ...
            <ExecutionSettings v-model="draft.execution" :show-header="false" />
          </el-collapse-item>
        </el-collapse>
      </section>

      <section class="c4-create-page__section">
        <header class="c4-create-page__section-header"><div><h2>执行参数</h2><p>C4 仅使用产品代码这一维参数。</p></div></header>
        ...
        <div class="c4-create-page__option-grid">
          ...
          <el-form-item label="K 线复权"><el-select v-model="draft.klineAdjustment"><el-option label="前复权" value="forward" /><el-option label="后复权" value="back" /><el-option label="不复权" value="none" /></el-select></el-form-item>
        </div>
        <div class="c4-create-page__date-grid">
          ...
          <el-form-item label="开始日期"><el-date-picker v-model="draft.startDate" type="date" value-format="YYYY-MM-DD" placeholder="开始日期" /></el-form-item>
          <el-form-item label="结束日期"><el-date-picker v-model="draft.endDate" type="date" value-format="YYYY-MM-DD" placeholder="结束日期" /></el-form-item>
        </div>
      </section>
...
<style scoped>
.c4-create-page { display: grid; max-width: 1440px; gap: 16px; margin: 0 auto; }
.c4-create-page__header, .c4-create-page__section-header, .c4-create-page__actions { display: flex; align-items: end; justify-content: space-between; gap: 16px; }
.c4-create-page__header p, .c4-create-page__section p { margin: 0 0 2px; color: var(--admin-text-muted); font-size: 13px; }
.c4-create-page__header h1, .c4-create-page__section h2 { margin: 0; color: var(--admin-text); font-weight: 600; }
.c4-create-page__header h1 { font-size: 20px; line-height: 28px; }
.c4-create-page__section h2 { font-size: 16px; line-height: 24px; }
.c4-create-page__section p { margin-top: 4px; }
.c4-create-page__form, .c4-create-page__sheets { display: grid; gap: 16px; }
.c4-create-page__section, .c4-create-page__actions { padding: 20px; border: 1px solid var(--admin-border); border-radius: var(--admin-radius); background: var(--admin-surface); }
.c4-create-page__meta-grid { display: grid; grid-template-columns: minmax(180px, 0.8fr) minmax(220px, 1fr) minmax(260px, 1.3fr); gap: 16px; }
.c4-create-page__description { grid-column: span 1; }
.c4-create-page__option-grid, .c4-create-page__date-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px 16px; }
.c4-create-page__date-grid { grid-template-columns: minmax(220px, 1.3fr) repeat(2, minmax(180px, 1fr)); }
.c4-create-page__section :deep(.el-form-item) { margin-bottom: 0; }
.c4-create-page__date-grid :deep(.el-date-editor), .c4-create-page__option-grid :deep(.el-select) { width: 100%; }
.c4-create-page__execution-collapse { border-top: 1px solid var(--admin-border-light); border-bottom: 1px solid var(--admin-border-light); }
.c4-create-page__execution-collapse :deep(.el-collapse-item__header), .c4-create-page__execution-collapse :deep(.el-collapse-item__wrap) { border-bottom: 0; background: transparent; }
.c4-create-page__execution-collapse :deep(.el-collapse-item__header) { height: 56px; }
.c4-create-page__execution-collapse :deep(.el-collapse-item__content) { padding-bottom: 16px; }
.c4-create-page__execution-title { display: flex; align-items: center; gap: 10px; }
.c4-create-page__execution-title > .el-icon { color: var(--admin-primary); font-size: 18px; }
.c4-create-page__execution-title div { display: grid; gap: 1px; }
.c4-create-page__execution-title strong { color: var(--admin-text); font-size: 14px; font-weight: 600; }
.c4-create-page__execution-title span { color: var(--admin-text-muted); font-size: 12px; }
.c4-create-page__actions { align-items: center; }
.c4-create-page__actions > div { display: flex; flex-wrap: wrap; justify-content: end; gap: 8px; }
@media (max-width: 1000px) { .c4-create-page__meta-grid, .c4-create-page__option-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }.c4-create-page__description { grid-column: span 2; }.c4-create-page__date-grid { grid-template-columns: 1fr 1fr; }.c4-create-page__date-grid :deep(.el-form-item:first-child) { grid-column: 1 / -1; } }
@media (max-width: 640px) { .c4-create-page__header, .c4-create-page__section-header, .c4-create-page__actions { align-items: stretch; flex-direction: column; }.c4-create-page__section, .c4-create-page__actions { padding: 16px; }.c4-create-page__meta-grid, .c4-create-page__option-grid, .c4-create-page__date-grid { grid-template-columns: 1fr; }.c4-create-page__description, .c4-create-page__date-grid :deep(.el-form-item:first-child) { grid-column: auto; }.c4-create-page__actions > div { display: grid; grid-template-columns: 1fr 1fr; }.c4-create-page__actions > div :deep(.el-button:last-child) { grid-column: 1 / -1; } }
</style>
