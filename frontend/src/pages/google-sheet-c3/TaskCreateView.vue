...
const { templates, loading: loadingTemplates, loadTemplates: loadAvailableTemplates } = useTaskTemplates('google_sheet')
...
function copyConfig(config: Record<string, unknown>, name = '', description = '') {
  ...
  const nextDraft: GoogleSheetCreateDraft = {
    taskName: name || String(config.base_task_name || ''),
    description: description || String(config.task_description || ''),
    sheet: { spreadsheetId: String(config.spreadsheet_id || ''), title: String(config.title || config.spreadsheet_title || ''), sheetName: String(config.sheet_name || '') },
    ...
    parameters: Array.from({ length: 6 }, (_, index) => values[index] ? JSON.stringify(values[index]) : ''),
  }
...
async function submit() {
  try {
    ...
    void router.push({ name: 'C3TaskDetail', params: { taskId: response.task_id } })
...
async function saveTemplate() {
  try {
    ...
    saveTemplateVisible.value = false; ElMessage.success('模板已保存'); loadTemplates(true)
  ...
  <section class="create-page">
    ...
    <el-form class="create-page__form" label-position="top" @submit.prevent>
      <section class="create-page__section">
        <header class="create-page__section-header"><div><h2>任务信息</h2><p>填写任务名称，或从已保存的 C3 模板恢复参数。</p></div></header>
        <div class="create-page__fields create-page__fields--meta">
